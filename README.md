...
**傾けた分だけ効く。** 斜めに傾ければ斜めに転がり、手前や奥へ倒せば奥行き方向にも動く。
4 方向のカチカチではなく、端末の姿勢がそのまま世界の重力になる。

**振ると跳ねる。** 傾けるだけでは越えられない段差は、端末を振って玉を弾く。

障害物を避けながらゴールの輪へ運ぶ。

センサーが使えない環境（PC など）では、**画面のドラッグ**が傾きの代わりになる。
矢印キーでも傾けられ、スペースキーが「振る」に相当する。
...
- 5 ステージが実際にクリアできるか。斜め傾け・奥行き・振るをそれぞれ使って
- **素直な手では解けないことも確認する**
- OS が画面を自動回転させた状態でも成立するか
- 日本語と英語で文言が出るか
- センサーが無い環境で、ドラッグで遊べるか
...
src/
  core/
    ...
    shake.ts               端末を振ったことの検出。姿勢とは別の入力
  physics/
    world.ts               球と直方体の物理。3 次元、固定間隔で解く自作の最小実装
  stages/
  ...
  ui/
    gameUi.ts              ステージ番号・回転数・クリア表示
    tiltControls.ts        センサーが無い環境で、ドラッグを傾きに割り当てる
...
- **3 次元で解く。** 斜めに傾ければ斜めに、奥へ倒せば奥行き方向に動く。
...
**振るのは「必須」ではなく「近道」**。ステージ側もその前提で作っている。

「振らないと絶対に解けない」配置を作ろうとすると、天井沿いの迂回路まで塞ぐことになり、
結局ただの一本道になってしまう。手数で差がつく作りのほうがパズルとして成立する。
...
- ステージは 5 つだけ。仕組みは揃ったので、あとは増やすだけの状態。
- 音は無い。
